...
| 字段 | 类型 | 说明 |
|------|------|------|
| `retrieval_trace.queries` | string[] | 实际 ES 检索 query 列表 |
| `retrieval_trace.tier` | int \| null | fallback tier（0-based）；skip 时为 null |
| `retrieval_trace.hits_before_rerank` | int | rerank 前 chunk 数 |
| `retrieval_trace.hits_after_rerank` | int | rerank 后 chunk 数 |
| `retrieval_trace.page_expand_added` | int | 同页扩展新增 chunk 数 |
| `retrieval_trace.asr_expand_added` | int | 同 ASR 段扩展新增 chunk 数 |
| `chunk_snippets` | object[] | `{ref_id, filename, snippet}`，snippet 为正文摘要（默认 300 字） |

### 工具与 HITL（Phase B）

| 字段 | 类型 | 说明 |
|------|------|------|
| `tool_trajectory` | object[] | `{id, name, status, args?, output_preview?, retry_count?}` |
| `hitl_decisions` | object[] | `{decision, tool_name?, message?}`，resume 时写入 |
...
流式回答完成后，assistant 消息展示 👍 / 👎。`run_id` 经 SSE `trace` 事件下发；**Phase B 起** 亦写入 checkpoint，history API 重载后仍可反馈。
...
**限制说明**：Phase B 起 metadata 含 `chunk_snippets`（chunk 正文摘要）。Online Eval **优先使用 `chunk_snippets`** 做 faithfulness 判断；无 snippets 时回退 `chunk_refs` + trace inputs。

### Evaluator 1：`rag_faithfulness`

**输入变量**：`inputs`（用户问题）、`outputs`（助手答案）、run metadata **`chunk_snippets`**（优先）或 `chunk_refs`、`retrieval_plan.standalone_query`
...
检索到的 chunk 摘要（优先；每项含 ref_id 与 snippet 正文）：
{chunk_snippets}
...
## Phase B — Agent 决策链可观测

**目标**：LangSmith 上可见 router / 检索 / rerank / expand / grounding 各阶段子 span；turn metadata 含检索 trace、工具轨迹；日志与 trace 互跳。

### 配置

| 变量 | 默认 | 说明 |
|------|------|------|
| `LANGSMITH_METADATA_SNIPPET_CHARS` | 300 | `chunk_snippets` 单条最大字符 |
| `LANGSMITH_STAGE_TRACING_ENABLED` | true | 关闭则命名 span no-op（LC 自动 trace 仍保留） |

### 命名 Span 表

| Span 名 | 代码位置 | 主要 outputs |
|---------|----------|--------------|
| `rag_retrieval_router` | `plan_retrieval` | action, strategy, standalone_query |
| `rag_es_search` | `search_with_plan` | tier, queries, hits |
| `rag_rerank` | `_apply_rerank` | hits_after_rerank |
| `rag_page_expand` | `_expand_same_page_chunks` | expanded_count |
| `rag_asr_expand` | `_expand_same_asr_segment_chunks` | expanded_count |
| `rag_grounding_judge` | `validate_grounding` | status, supported_ratio |

LangSmith UI 过滤示例：`has(tags, "rag_stage")` 或按 span 名 `rag_es_search`。

### run_id 持久化

每轮 assistant 回复的 `run_id` / `trace_id` 写入 LangGraph state `message_traces`，经 `GET /api/chat/history` 返回，前端可补交反馈。

### 结构化日志

所有应用日志含固定字段（由 `TraceContextFilter` 注入）：

```
trace_id=... session_id=... user_id=...
```

chat turn 开始后 `trace_id` 在捕获 root run 后更新；可与 LangSmith run URL 互跳。
...
| 模块 | 职责 |
|------|------|
| `app/observability/stage_trace.py` | Phase B 命名 span 装饰器 |
| `app/observability/retrieval_trace.py` | 检索 trace dataclass |
| `app/observability/log_context.py` | 结构化日志 trace/session/user |
