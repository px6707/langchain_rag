...
from app.parsing.parse_execution import ParseExecutionContext
...
def test_run_parse_pipeline_python_strategy():
    ...
    with (
        ...
        patch.object(ParseExecutionContext, "abort_if_lost", new_callable=AsyncMock),
        patch("app.parsing.pipeline.index_documents", new_callable=AsyncMock, return_value=1) as mock_index,
    ):
        count = asyncio.run(run_parse_pipeline(session, document, ctx))
